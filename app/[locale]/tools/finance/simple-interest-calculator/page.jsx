...
export async function generateMetadata({ params }) {
  ...
  const sicCalc = (await import(`../../../../../messages/${locale}/sicCalc.json`).catch(() => ({}))).default || {};

  // use the seo block from sicCalc.json (user provided)
  const pageSeo = sicCalc.seo || {};

  // build opts for createMetadata (your lib/seo.js expects similar keys)
  const opts = {
    title: pageSeo.title || sicCalc.site?.heading || common.site?.name || SITE.name,
  ...
  for (const lng of SITE.locales) {
    const other = (await import(`../../../../../messages/${lng}/sicCalc.json`).catch(() => ({}))).default || {};
